window.addEventListener('DOMContentLoaded', function () {
    ...
    var originBtn = urlParams.get('option');
    // console.log(ogirinBtn);

    var totalSize = 0;
    var selectedImages = []; // Array to store selected images
    var imgOptionData = []; // Array to store selected images
    var storedData;

    var imgOption = (data) => {
        if (originBtn === 'img') {
            imgOptionData = []; // reset imgOptionData
            imgOptionData.push(data[data.length - 1]);
            document.querySelectorAll('.img-media').forEach((eImg) => {
                if (eImg.src !== imgOptionData[0].src) {
                    eImg.classList.remove('img-selected');
                }
            });
            // console.log(imgOptionData);
            // console.log(selectedImages);
            selectedImages = [];
            imgOptionData = imgOptionData.slice();
    ...
    var render = () => {
        ...
        // public/dashboard/inno-media/assets/php/get_images.php
        fetch('/picture/assets/php/get_images.php')
    ...
    function handleFileUpload(event) {
        ...
        fetch("/picture/assets/php/upload.php", {
            method: "POST",
            body: formData,
        })
            .then((response) => {
    ...
    function deleteImage(imageSrc) {
        ...
        fetch('/picture/assets/php/delete_image.php', {
            method: 'POST',
            headers: {
    ...
    if (imageSelectedBtn) {
        imageSelectedBtn.addEventListener("click", () => {
            if (selectedImages.length > 0) {
                console.log("Album ảnh đã chọn: ", selectedImages);
                const message = {
                    type: "album",
                    data: selectedImages
                };
                ...
                window.close();
            } else if (imgOptionData) {
                console.log("Hình ảnh đã chọn: ", imgOptionData);
                const message = {
                    type: "img",
                    data: imgOptionData
                };
